...
public class StringUtils {
	 ...
	 * @param pattern
	 *            正则表达式
	 * @return 是否匹配。true-匹配；false-不匹配或参数不合法
	...
	/**
	 * 分转元 最多保留2位小数,如果最后小数为0则删除 1004 -> 10.04
	 * 
	 * @param price
	 * @return
	 */
	public static String formatMoney(int price) {
		...
		if (lastChar1 == '0') {
			stringBuilder.deleteCharAt(stringBuilder.length() - 1);// 删除最后第一个0
			if (lastChar2 == '0') {
				stringBuilder.deleteCharAt(stringBuilder.length() - 1);// 删除最后第二个0
				stringBuilder.deleteCharAt(stringBuilder.length() - 1);// 删除小数点
			}
		}
	...
	// 不能是汉字 [^\u4e00-\u9fa5] //^[0-9A-Za-z^!#%&',;=+*/?$_-]{6,20}$

	static String MAIL = "\\w+([-+.]\\w+)*@\\w+([-.]\\w+)*\\.\\w+([-.]\\w+)*";
	static String PHONE = "^1[23456789]\\d{9}$";
	...
	// 把 123545@qq.com 处理成12****5@qq.com
