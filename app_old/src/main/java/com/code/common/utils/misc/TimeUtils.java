...
public class TimeUtils {
	...
	public static long getTslong() {
		...
		try {
			...
			conn.connect();
			ts =  conn.getDate();
			FLogger.w(FLogger.UTIL_TAG, "获取到的百度时间:" + ts);
			if(ts == 0) {
				ts =  System.currentTimeMillis();
				FLogger.w(FLogger.UTIL_TAG, "时间更正.."+ts);
			}
			conn.disconnect();
		} catch (Exception e) {
			FLogger.Ex(FLogger.UTIL_TAG, e);
			ts = System.currentTimeMillis();
			FLogger.w(FLogger.UTIL_TAG, "获取到的本地时间:" + ts);
	...
	public static boolean checkDownTime(String startDate, String endDate){
		...
		try {
			...
			FLogger.w(FLogger.UTIL_TAG, "startTime:"+startTime+" currentTime:"+currentTime+" endTime:"+endTime);
