                                        ...
                                        jintArray arr)
{
  ...
  p[0] = 999;
  //    env->ReleaseIntArrayElements(arr,p,0);//若ReleaseIntArrayElements传0,把修改后的数组拷贝覆盖给原数组，然后释放现在的数组指针p
  ...
  p2[0] = 11;
  p2[1] = 12;
  env->ReleaseIntArrayElements(arr2, p2, 0);
...
*/
  ...
  jmethodID  mid = env->GetStaticMethodID(clazz,"testFun3","(Ljava/lang/String;ID)V");// I int D double
  if(mid == nullptr){
    LOGI("没有找到方法");
    return;
  }
